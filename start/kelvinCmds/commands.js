...
async function playCommand(kelvin, chatId, message, args) {
    try {
        const text = args.join(' ').trim();
        
        if (!text) return kelvin.sendMessage(chatId, { 
...
_Reply with 1, 2 or 3 to this message to download the format you prefer._`;
            ...
            const songmsg = await kelvin.sendMessage(chatId, { 
                text: formatMenu 
            }, { quoted: message });
            ...
            const selectionHandler = async (msgUpdate) => {
                try {
                    ...
                    if (
                        ...
                        await kelvin.sendMessage(chatId, { react: { text: "⬇️", key: mp3msg.key } });
...
async function takeCommand(kelvin, chatId, message, args) {
    try {
        // Check if message is a reply to a sticker
        const quotedMessage = message.message?.extendedTextMessage?.contextInfo?.quotedMessage;
        if (!quotedMessage?.stickerMessage) {
            await kelvin.sendMessage(chatId, { text: '❌ Reply to a sticker with .take <packname>' });
            return;
        }
        ...
        try {
            ...
            const stickerBuffer = await downloadMediaMessage(
                ...
                {
                    logger: console,
                    reuploadRequest: kelvin.updateMediaMessage
                }
            );

            if (!stickerBuffer) {
                await kelvin.sendMessage(chatId, { text: '❌ Failed to download sticker' });
                return;
            }
            ...
            await kelvin.sendMessage(chatId, { text: '❌ Error processing sticker' });
        }

    } catch (error) {
        console.error('Error in take command:', error);
        await kelvin.sendMessage(chatId, { text: '❌ Error processing command' });
    }
}

async function videoCommand(kelvin, chatId, message) {
    try {
        ...
        const processingMsg = await kelvin.sendMessage(chatId, { 
        ...
        const infoMsg = await kelvin.sendMessage(chatId, { text: caption }, { quoted: message });
...
async function ytplayCommand(kelvin, chatId, query, message) {
    if (!query) {
        return await kelvin.sendMessage(chatId, {
    ...
    try {
        ...
        if (!query.includes("youtube.com") && !query.includes("youtu.be")) {
            ...
            if (!search.videos || search.videos.length === 0) {
                return await kelvin.sendMessage(chatId, { text: `❌ No results found for: ${query}` });
            }
        ...
        if (!data || !data.download_url) {
            await kelvin.sendMessage(chatId, { react: { text: "❌", key: message.key } });
            return await kelvin.sendMessage(chatId, { text: "❌ Failed to fetch audio. Try another link." });
        }
        ...
        await kelvin.sendMessage(chatId, { react: { text: "❌", key: message.key } });
        await kelvin.sendMessage(chatId, { text: "❌ An error occurred while processing your request." });
    }
}


async function InstagramCommand(kelvin, chatId, message) {
    try {
        const text = message.message?.conversation || message.message?.extendedTextMessage?.text;
        const args = text.split(' ').slice(1); // Remove command prefix
...
async function handleMediafireDownload(kelvin, chatId, message) {
    try {
        const text = message.message?.conversation || message.message?.extendedTextMessage?.text;
        const args = text.split(' ').slice(1); // Remove command prefix
